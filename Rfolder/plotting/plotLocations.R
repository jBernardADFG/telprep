...
#'
#' Plot fish locations
#' @param spdf Shapefile or SpatialLinesDataFrame representation of the river network
...
#' @export
plot.locations <- function(spdf, detects, col_by_fish=T, flight_num=NA, channel=NA, tag_id=NA, main=""){
  ...
  par(mar=c(1,1,1,1))
  plot(crop(spdf,extent(sldf)), main=main)
  rect(spdf@bbox[1,1],spdf@bbox[2,1],spdf@bbox[1,2],spdf@bbox[2,2], col = "black")
  # lines(crop(spdf,extent(sldf)), col="back", lwd=1)
  ...
  x_vec <- c(spdf@bbox[1,1],spdf@bbox[1,1],spdf@bbox[1,2],spdf@bbox[1,2],spdf@bbox[1,1])
  y_vec <- c(spdf@bbox[2,1],spdf@bbox[2,2],spdf@bbox[2,2],spdf@bbox[2,1],spdf@bbox[2,1])
  ...
  if (col_by_fish){
    ...
    if(sum(names(detects)=="MortFlag")==0){
      lines(crop(spdf,extent(sldf)), col="blue4", lwd=2)
      points(bd$Longitude, bd$Latitude, pch=19, col=cols, cex=1, xlim=bbox(spdf)[1,],ylim=bbox(spdf)[2,])
    }else{
      ...
      print(plot_sym)
      lines(crop(spdf,extent(sldf)), col="blue4", lwd=2)
      points(bd$Longitude, bd$Latitude, pch=plot_sym, col=cols, cex=1, xlim=bbox(spdf)[1,],ylim=bbox(spdf)[2,])
    }
  }else{
    if(sum(names(detects)=="MortFlag")==0){
      lines(crop(spdf,extent(sldf)), col="blue4", lwd=2)
