...
XX Add a little info on the context of why this program was developed -- this should include some description of the basic experimental design. Needs to include model information of the tags/receivers XX

This vignette will show you how the telprep package can be used to automatically process data from telemetry flights. The program is designed to read in raw txt files, filter out erroneous signals, determine the living status of fish, and act as a strava heatmap, but for fish! This tutorial highlights the general workflow of the program. For a finer scale description of the program functionalities, see the help files for the following functions: read.flight.data, channels.merge, replace.date, combine.data, get.date.bins, get.best.locations, flag.dead.fish, and add.to.database. XX Make sure that all functions are listed XX


### Program Workflow:
* Step 1 -- Import river line data into R
* Step 2 -- Read detection data into R 
...
* Step 5 -- Determine the best locations for each fish in each detection period and remove false detections
* Step 6 -- Determine the living status fish
* Step 7 -- Create some pretty plots
* Step 8 -- Convert the data into the appropriate structure to interface with the riverdist package
...
### Step 5: Determine the best locations and flag for erroneous signals
...
XX Need to account for fish that are undetected -- what is the tag length --
...
### Step 7 -- Interfacing with the riverdist package

### Step 8 -- Add the results to an existing database
