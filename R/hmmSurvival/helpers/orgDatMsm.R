...
org.dat.msm <- function(best_detects){
  ...
  r_df$Displacement <- r_df$Displacement>1
  r_df$TagStatus <- round(as.numeric(r_df$TagStatus))
  for (i in 1:nrow(r_df)){
    if (is.na(r_df$Displacement[i])){
      r_df$Displacement[i] <- 1
    } else if (r_df$Displacement[i]){
      r_df$Displacement[i] <- 2
    } else{
      r_df$Displacement[i] <- 3
    }
  }
  new_tag_status <- rep(2, nrow(r_df))
  new_tag_status[r_df$TagStatus=="Mort"] <-3
  r_df$TagStatus <- new_tag_status
  ...
  for (i in 1:nrow(r_df)){
    s <- r_df$Subject[i]
    d <- r_df$Day[i]
    if(!is.na(r_df$Displacement[r_df$Subject==s & r_df$Day==d])){
      obs1 <- r_df$Displacement[r_df$Subject==s & r_df$Day==d]
    }
    if(!is.na(r_df$TagStatus[r_df$Subject==s & r_df$Day==d])){
      obs2 <- r_df$TagStatus[r_df$Subject==s & r_df$Day==d]
    }
